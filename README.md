...
が必要です。無い場合は先にインストールを済ませてください。

以下のコマンドを実行してください。インストールされます。
```bash
pip install git+https://github.com/Nasu726/RtoJ-anki-register.git
```

インストールが終わったら、以下のコマンドを打つことで利用できます。
```bash
rj-anki
```
初回起動時に設定事項があるので、画面に表示される指示に従って設定を済ませてください。設定が済んだら、すぐに利用できます。
